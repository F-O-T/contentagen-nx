...
function useStore$2(store, selector = (d) => d, options = {}) {
  const equal = options.equal ?? shallow$2;
...
function shallow$2(objA, objB) {
  ...
  const keysA = getOwnKeys$2(objA);
  if (keysA.length !== getOwnKeys$2(objB).length) {
    return false;
  }
...
function shallow$1(objA, objB) {
  ...
  if (keysA.length !== getOwnKeys$1(objB).length) {
    return false;
  }
  for (let i = 0; i < keysA.length; i++) {
    if (!Object.prototype.hasOwnProperty.call(objB, keysA[i]) || !Object.is(objA[keysA[i]], objB[keysA[i]])) {
