							...
							<TableRow key={event.id}>
								<TableCell>
									<div>
										...
										<p className="text-xs text-muted-foreground">{event.displayName}</p>
									</div>
								</TableCell>
								<TableCell>
									<Badge variant="secondary">{event.category}</Badge>
								</TableCell>
								<TableCell className="text-right font-mono text-sm">R${event.pricePerEvent}</TableCell>
								<TableCell className="text-right tabular-nums">
									{event.freeTierLimit > 0 ? `${event.freeTierLimit.toLocaleString("pt-BR")}/mo` : "-"}
								</TableCell>
								<TableCell>
