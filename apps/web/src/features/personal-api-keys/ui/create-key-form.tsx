...
export function CreateKeyForm({ onSuccess }: CreateKeyFormProps) {
	...
	function handleCreate() {
		...
		createMutation.mutate({
			...
			scopes,
			organizationAccess:
				orgAccessMode === "all" ? "all" : selectedOrgIds,
		});
	}
				...
				<div className="space-y-2 px-1">
					...
					>
						...
						<TabsContent value="specific">
							<p className="text-xs text-muted-foreground mt-2">
								Selecione organizações específicas para limitar o
								acesso desta chave. (Em breve)
							</p>
						</TabsContent>
					</Tabs>
				</div>
						...
						{filteredDefinitions.map((def) => (
							...
							>
								...
								<ToggleGroup
									className="shrink-0"
									onValueChange={(v) => {
										if (v) handleScopeChange(def.resource, v as ScopeAccess);
									}}
									size="sm"
									type="single"
