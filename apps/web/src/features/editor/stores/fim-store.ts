/**
 * FIM (Fill-in-Middle) Store
 *
 * TanStack Store for managing FIM completion state.
 * Handles ghost text, confidence scoring, diff mode, and chaining.
 */
...
/**
 * Subscribe to FIM state changes
 */
export const subscribeFIM = (listener: (state: FIMState) => void) => {
   return fimStore.subscribe(() => listener(fimStore.state));
};
