...
import { checkCompletionSpelling } from "../lib/spell-checker-client";
import {
   $createGhostTextNode,
   $isGhostTextNode,
   type GhostTextNode,
...
export function FIMPlugin({ containerRef }: FIMPluginProps) {
   ...
   const handleAcceptSuggestion = useCallback(() => {
      ...
      if (
         mode === "diff" &&
         diffSuggestion?.type === "replace" &&
         diffSuggestion.replaceRange
      ) {
         editor.update(() => {
   ...
   const handleComplete = useCallback(
      (fullText: string, metadata?: FIMChunkMetadata) => {
         ...
         if (metadata && !metadata.shouldShow) {
            console.log(
               "[FIM] Hiding suggestion due to low confidence:",
               metadata.confidence,
            );
            handleClearFIM();
            return;
         ...
         const diff = detectDiffType(
            lastPrefixRef.current,
            lastSuffixRef.current,
            fullText,
         );
         ...
         if (
            currentModeRef.current === "cursor-tab" ||
            currentModeRef.current === "diff"
         ) {
   ...
   const triggerCompletion = useCallback(
      (
         ...
         editor.getEditorState().read(() => {
            ...
            if (
               options.triggerType !== "chain" &&
               textContent === lastTextRef.current
            ) {
               clearFIM();
               return;
            }
            ...
            const { prefix, suffix } = buildFIMContext(
               textContent,
               cursorOffset,
            );
