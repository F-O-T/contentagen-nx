...
export function useFIMTriggers({
   ...
   const cursorMoveTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
      null,
   );
   const editPredictionTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
      null,
   );
   ...
   useEffect(() => {
      ...
      return editor.registerCommand(
         ...
         () => {
            ...
            if (config.enableEditPrediction) {
               const isMidText =
                  textLength > 0 && cursorOffset < textLength * 0.85;
               const cursorMoved =
      ...
      editor,
      enabled,
      config.enableCursorMove,
      config.enableEditPrediction,
      config.cursorMoveDebounceMs,
      config.editPredictionDelayMs,
      onTrigger,
   ]);
   ...
   return {
      ...
      cancelTriggers,
      scheduleDebounceTrigger: (context: TriggerContext) =>
         scheduleDebounceTrigger(context),
   };
}
