...
/**
 * Pre-computed ignore set (created once at module load)
 */
const IGNORE_SET = new Set(IGNORE_LIST.map((w) => w.toLowerCase()));

/**
 * Browser storage for dictionary caching (IndexedDB + Cache API in workers)
 */
const dictionaryStorage = createDictionaryStorage({
	keyPrefix: "contentta-spelling",
	sessionTTL: 60 * 60 * 1000, // 1 hour (not used in workers)
	...
	try {
		...
		for await (const error of checkerInstance.checkTextStream(text)) {
			...
			if (batch.length >= BATCH_SIZE) {
				postResponse({ type: "check-progress", id, errors: batch, done: false });
				batch = [];
...
initializeSpellChecker().catch(console.error);
