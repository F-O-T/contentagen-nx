...
describe("Performance Benchmarks", () => {
	...
	beforeAll(async () => {
		...
		if (!fs.existsSync(affPath) || !fs.existsSync(dicPath)) {
			console.log("Skipping benchmarks - dictionary not found at", dictionaryPath);
			return;
		}
	...
	it("should check single word in < 1ms", () => {
		...
		const testWords = ["palavra", "computador", "programação", "desenvolvimento", "inexistente"];
	...
	it("should benefit from caching on repeated checks", () => {
		...
		console.log(`Cache stats: ${stats.checkHits} hits, ${stats.checkMisses} misses`);

		// Should have mostly cache hits after first check
		expect(stats.checkHits).toBeGreaterThan(iterations - 10);
		console.log(`✓ Cache hit rate: ${((stats.checkHits / iterations) * 100).toFixed(1)}%`);
	});

	it("should generate suggestions in < 200ms", () => {
		if (!checker) return;

		const misspelledWords = ["palavrra", "computadro", "programaçao", "desenvolviment"];
		const iterations = 10;
	...
	it("should check text with 1000 words in < 5000ms (with suggestions)", () => {
		...
		const words = [
			"o", "que", "é", "um", "em", "para", "com", "não", "uma", "os",
			"palavra", "casa", "computador", "programa", "sistema", "trabalho",
			"desenvolvimento", "aplicação", "usuário", "dados", "informação",
		];
		...
		console.log(`✓ Text check (${result.wordCount} words): ${checkTime.toFixed(2)}ms`);
	...
	it("should handle incremental check efficiently", () => {
		...
		console.log(`✓ Incremental check: ${checkTime.toFixed(2)}ms (${result.wordCount} words in region)`);
	});
	...
	it("should stream errors asynchronously", async () => {
		...
		console.log(`✓ Stream check: ${streamTime.toFixed(2)}ms, found ${errors.length} errors`);
	});
	...
	it("should check 3000-word document in < 500ms (streaming, no suggestions)", async () => {
		...
		const repeatCount = Math.ceil(targetWords / (paragraphs.length * wordsPerParagraph));
		...
		console.log(`✓ Large document stream (${wordCount} words): ${streamTime.toFixed(2)}ms`);
	...
	it("should check 3000-word document synchronously in reasonable time", () => {
		...
		let text = "";
		for (let i = 0; i < 300; i++) { // ~3000 words
		...
		console.log(`✓ Large document sync (${result.wordCount} words): ${checkTime.toFixed(2)}ms`);
		console.log(`  Internal timing: ${result.checkTimeMs.toFixed(2)}ms`);
		console.log(`  Errors found: ${result.errors.length}`);
	});
});
