...
describe('Agent System Prompt Assembler', () => {
   const mockPersonaConfig: PersonaConfig = {
      metadata: {
         ...
         description: "An AI specialized in creating engaging marketing content that converts and builds brand awareness."
      },
      voice: {
   ...
   const mockOptions: PromptOptions = {
      contentRequest: {
         ...
         briefDescription: "Write a comprehensive blog post about the top digital marketing trends for 2024"
      },
      additionalContext: "Focus on AI integration and privacy-first marketing",
      specificRequirements: ["Include case studies", "Mention ROI considerations", "Add actionable tips"]
   };
   ...
   describe('Individual Section Creators', () => {
      it('should create metadata section correctly', () => {
         ...
         expect(section).toContain('AI Assistant Identity: Content Marketing Expert');
   ...
   describe('Full System Prompt Generation', () => {
      it('should generate complete system prompt with all sections', () => {
         ...
         expect(prompt).toContain('AI Assistant Identity: Content Marketing Expert');
   ...
   describe('Template Structure', () => {
      it('should provide complete template structure', () => {
         ...
         expect(structure.audience).toEqual(['general_public', 'professionals', 'beginners', 'customers']);
         expect(structure.formatting).toEqual(['structured', 'narrative', 'list_based']);
         expect(structure.language).toContain('base');
         expect(structure.brand).toEqual(['strict_guideline', 'flexible_guideline', 'reference_only', 'creative_blend']);
         expect(structure.purpose).toEqual(['blog_post', 'linkedin_post', 'twitter_thread', 'instagram_post', 'email_newsletter', 'reddit_post', 'technical_documentation']);
   ...
   describe('Validation Functions', () => {
      ...
      it('should validate content length limits', () => {
         ...
         expect(errors).toContain('Topic should be under 200 characters for clarity');
         expect(errors).toContain('Brief description should be under 1000 characters');
      });
   });
   ...
   describe('Preview Sections', () => {
      it('should generate preview sections correctly', () => {
         ...
         expect(preview).toContain('Persona Identity: Content Marketing Expert');
   ...
   describe('Edge Cases and Error Handling', () => {
      it('should handle different language variants correctly', () => {
         const configs = [
            { primary: "en" as const, variant: "en-GB" as const, expected: "British English" },
            { primary: "pt" as const, variant: "pt-BR" as const, expected: "Brazilian Portuguese" },
            { primary: "es" as const, variant: "es-ES" as const, expected: "Spain Spanish" }
         ];
      ...
      it('should handle all brand integration styles', () => {
         const styles = ["strict_guideline", "flexible_guideline", "reference_only", "creative_blend"] as const;
