...
function loadTemplate(category: string, filename: string): string {
   ...
   try {
      const filePath = join(__dirname, '..', 'prompt-files', category, `${filename}.md`);
...
type TemplateVariables = Record<string, string | boolean | string[] | undefined>;
   ...
   Object.entries(variables).forEach(([key, value]) => {
      if (Array.isArray(value)) {
         ...
         result = result.replace(arrayRegex, (_, template) => {
            return value.map(item => template.replace(/{{\.}}/g, String(item))).join('\n');
         });
      }
   });
...
export function createFormattingSection(config: PersonaConfig): string {
   ...
   if (config.formatting.style === 'list_based' && config.formatting.listStyle) {
      variables.listStyle = config.formatting.listStyle === 'bullets' ? 'bullet points (•)' : 'numbered lists (1, 2, 3)';
   }
...
export function validateContentRequest(request: ContentRequest): string[] {
   ...
   if (!request.briefDescription || request.briefDescription.trim().length === 0) {
...
export function previewPromptSections(personaConfig: PersonaConfig): string[] {
   ...
   if (personaConfig.voice?.communication) {
      sections.push(`Voice: ${personaConfig.voice.communication.replace('_', ' ')}`);
   }
   ...
   if (personaConfig.brand?.integrationStyle) {
      sections.push(`Brand Integration: ${personaConfig.brand.integrationStyle.replace('_', ' ')}`);
   }
...
export function generateTemplateStructure(): Record<string, string[]> {
   return {
      ...
      language: ['base'],
      brand: ['strict_guideline', 'flexible_guideline', 'reference_only', 'creative_blend'],
      purpose: ['blog_post', 'linkedin_post', 'twitter_thread', 'instagram_post', 'email_newsletter', 'reddit_post', 'technical_documentation'],
      task: ['base']
   };
}