...
import { type AgentInsert, agent } from "../schemas/agent";
...
export async function getAgentsByOrganizationId(
   ...
   try {
      ...
      propagateError(err);
      throw AppError.database(
         `Failed to get agents: ${(err as Error).message}`,
      );
   }
}
...
export async function deleteAgent(dbClient: DatabaseInstance, agentId: string) {
   try {
      const result = await dbClient
         .delete(agent)
         .where(eq(agent.id, agentId))
