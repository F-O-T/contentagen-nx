...
import { suggestionPlugin } from "@packages/ui/components/editor/plugins/suggestion-kit";
...
export const useResolveSuggestion = (
   ...
   const resolvedSuggestion: ResolvedSuggestion[] = React.useMemo(() => {
      ...
      suggestionIds.forEach((id) => {
               ...
               if (lineBreakData?.id === keyId2SuggestionId(id)) {
                  if (lineBreakData.type === "insert") {
                     newText += lineBreakData.isLineBreak
                        ? BLOCK_SUGGESTION
                        : // biome-ignore lint/style/noNonNullAssertion: scaffolded Plate.js pattern, node.type guaranteed to exist in TYPE_TEXT_MAP
         ...
         const nodeData = firstEntry
            ? api.suggestion.suggestionData(firstEntry[0])
            : undefined;
