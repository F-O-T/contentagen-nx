...
import { z } from "zod";
import type { IdeaContentSchema, ConfidenceScoreSchema } from "@packages/database/schema";
...
// Helper function to update idea status and emit server events
async function updateIdeaStatus(
  ...
  try {
    ...
    // Note: We're not updating the idea in DB here as it's already created
    // This is just for status tracking during generation
  } catch (err) {
    console.error(
      "[IdeaGeneration] Failed to update idea status in DB:",
      err,
    );
  }
  emitIdeaStatusChanged({ ideaId, status, message });
}
...
// Step 1: Generate ideas using the ideas-agent
const generateIdeasStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const { agentId, keywords, userId, language } = inputData;

    console.log(`[GenerateIdeas] Starting idea generation for agent ${agentId} with keywords: ${keywords.join(", ")}`);

    // Set runtime context for language
...
`;
    ...
    console.log(`[GenerateIdeas] Successfully generated ${generatedIdeas.length} ideas`);
...
const saveIdeasToDatabase = createStep({
  ...
  execute: async ({ inputData }) => {
    const { agentId, generatedIdeas, userId } = inputData;
    const db = createDb({ databaseUrl: serverEnv.DATABASE_URL });
    const ideaIds: string[] = [];

    console.log(`[SaveIdeas] Saving ${generatedIdeas.length} ideas to database for agent ${agentId}`);
    ...
    for (const idea of generatedIdeas) {
      ...
      try {
        ...
        console.log(`[SaveIdeas] Created idea ${createdIdea.id} with title: "${idea.title}"`);
      } catch (error) {
        console.error(`[SaveIdeas] Failed to save idea: "${idea.title}"`, error);
        // Continue with other ideas even if one fails
      }
    }

    console.log(`[SaveIdeas] Successfully saved ${ideaIds.length} ideas to database`);
