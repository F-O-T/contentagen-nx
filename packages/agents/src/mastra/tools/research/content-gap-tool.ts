      ...
      try {
         ...
         const gapPatterns = [
            ...
            },
            {
               ...
               description:
                  "Add templates, checklists, or downloadable resources",
            },
            {
            ...
            },
            {
               ...
               description:
                  "Add statistics, research data, or study references",
            },
         ];

         // Check which patterns are missing across all competitors
         const allHeadings = competitorData
         ...
         if (commonTopics.some((t) => /step|how|guide/i.test(t.topic))) {
            ...
            commonTopics.some((t) => /\d+|list|best|top/i.test(t.topic))
         ) {
         ...
         if (highValueUnique.length > 0) {
            recommendations.push(
               `Consider unique angles: ${highValueUnique
                  .map((t) => t.topic)
                  .slice(0, 3)
                  .join(", ")}`,
            );
