...
export const relatedKeywordsTool = createTool({
	...
	execute: async (inputData) => {
		...
		try {
			...
			const wordFrequency: Record<string, { count: number; sources: Set<string> }> = {};
			...
			for (const result of allResults) {
				...
				if (
					...
					title.split(/\s+/).length > primaryKeyword.split(/\s+/).length + 2
				) {
			...
			for (const result of [...questionResults.results, ...howToResults.results]) {
			...
			const modifierPatterns = [
				...
				{ pattern: /\d{4}/i, modifier: new Date().getFullYear().toString() },
