	...
	execute: async (inputData) => {
		...
		if (completedResearch.serpAnalysis) {
			...
			breakdown.push({
				...
				earned: 0,
				status: "missing",
				suggestion: "Call serpAnalysis to understand what content ranks well",
			});
		}
		...
		if (completedResearch.competitorCount >= 2) {
			...
			breakdown.push({
				...
				status: "partial",
				suggestion: "Analyze at least one more competitor for better insights",
			});
		...
		let qualityLevel: "insufficient" | "basic" | "good" | "excellent" | "comprehensive";
...
export function getResearchCompletenessInstructions(): string {
	return `
## RESEARCH COMPLETENESS TOOL
Evaluates if your research is thorough enough to create a quality plan.

**When to use:** MUST be called BEFORE createPlan. This is a required checkpoint.

**Parameters:**
- completedResearch (object):
  - serpAnalysis (boolean): Did you call serpAnalysis?
  - competitorCount (number): How many competitorContent analyses?
  - relatedKeywordsSearched (boolean): Did you call relatedKeywords?
  - contentGapsAnalyzed (boolean): Did you call contentGapFinder?
  - factsGathered (number): How many facts from factFinder?
  - deepCrawlCount (number): How many webCrawl calls?
- topicComplexity (enum): "simple", "moderate", or "complex"

**Returns:**
- score: Research completeness (0-100)
- canProceed: Whether you can call createPlan
- breakdown: Points earned for each research area
- missing: What research is still needed
- recommendations: Specific actions to improve research
- qualityLevel: Overall research quality rating

**Scoring:**
| Criterion | Points |
|-----------|--------|
| SERP analysis | 20 |
| 2+ competitor analyses | 20 |
| Related keywords | 15 |
| Content gaps | 15 |
| Facts gathered | 15 |
| Deep crawl | 15 |
...
**Minimum scores:**
- Simple topic: 60 points
- Moderate topic: 70 points
- Complex topic: 80 points

**IMPORTANT:** If canProceed is false, you MUST do more research before calling createPlan.
`;
}
