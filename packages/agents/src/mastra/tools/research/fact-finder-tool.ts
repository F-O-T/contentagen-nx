		...
		try {
			...
			const getCredibility = (url: string): "high" | "medium" | "low" => {
				try {
					...
					if (reputableSites.some((d) => domain.includes(d))) return "medium";
				...
				for (const sentence of sentences) {
					...
					switch (type) {
						case "statistics":
							...
							if (/\d+%|\$[\d,]+|\d+\s*(million|billion|thousand)/i.test(s)) {
								return s;
							}
							break;
						case "studies":
							// Look for research-related keywords
							if (/study|research|found|showed|according|published/i.test(s)) {
								return s;
							}
							break;
						case "quotes":
							// Look for quote patterns
							if (/said|says|according to|noted|explained|"[^"]+"/i.test(s)) {
								return s;
							}
							break;
						case "examples":
							// Look for example patterns
							if (/example|case|such as|like|including|for instance/i.test(s)) {
								return s;
							}
							break;
					}
			...
			for (const { type, results } of searchResults) {
				for (const result of results) {
					...
					const year = extractYear(result.snippet) || extractYear(result.title);
			...
			const sourcesSummary = {
				...
				highCredibility: selectedFacts.filter((f) => f.credibility === "high")
					.length,
