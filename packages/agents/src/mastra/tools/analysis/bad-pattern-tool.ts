...
export const badPatternTool = createTool({
   id: "bad-pattern-detector",
   description:
      "Detects common bad writing patterns in blog posts such as word count mentions, meta-commentary, engagement begging, and filler phrases.",
   ...
   execute: async (inputData) => {
      ...
      const patterns: z.infer<typeof PatternSchema>[] = [];

      // Helper function to find occurrences
      const findOccurrences = (regex: RegExp, text: string): string[] => {
      ...
      if (firstH2Index > 0) {
         ...
         if (introWords > 150) {
            patterns.push({
               ...
               severity: "warning",
               locations: [
                  `Introduction: ~${introWords} words before first H2`,
               ],
               suggestion:
      ...
      // 8. Over-formatting (excessive bold/italic in succession)
      ...
      // 10. Keyword stuffing (same phrase > 3% density)
