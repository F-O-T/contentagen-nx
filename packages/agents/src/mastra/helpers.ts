...
export function formatWriterConfig(config: WriterConfig | undefined): string {
   ...
   if (
      config.seoRules?.minKeywordDensity ||
      config.seoRules?.maxKeywordDensity
   ) {
   ...
   if (config.brandTerms && config.brandTerms.length > 0) {
      ...
      const alwaysUse = config.brandTerms.filter(
         (t) => t.usage === "always_use",
      );
      const neverUse = config.brandTerms.filter((t) => t.usage === "never_use");
      const preferred = config.brandTerms.filter(
         (t) => t.usage === "preferred",
      );
   ...
   if (config.enableFactChecking) {
      featureLines.push(
         "- Fact checking is ENABLED - verify claims with sources",
      );
   }
