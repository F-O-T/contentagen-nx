...
function runCommand(command: string, cwd: string = process.cwd()): Promise<void> {
...
function expandGlobPatterns(patterns: string[]): string[] {
  ...
  patterns.forEach(pattern => {
    try {
      ...
      const result = execSync(`find . -path "${pattern}" -not -path "./node_modules/*" 2>/dev/null || true`, {
        encoding: "utf8",
        cwd: process.cwd()
      });
...
async function clean(options: any) {
  ...
  if (options.dryRun) {
    console.log(colors.yellow(`🔍 DRY RUN: Would delete ${deletedCount} items`));
    console.log(colors.blue("💡 Run without --dry-run to actually delete these files"));
  } else {
    console.log(colors.green(`🎉 Cleanup complete! Deleted ${deletedCount} items`));
    if (skippedCount > 0) {
    ...
    if (rootNodeModulesDeleted && !options.dryRun && !options.noInstall) {
      ...
      const answer = await new Promise<string>((resolve) => {
        rl.question(colors.yellow("🤔 Would you like to reinstall dependencies? (y/N): "), resolve);
      });
...
async function reset(options: any) {
  ...
  console.log(colors.yellow("⚠️  This will delete ALL uncommitted changes and reset the repository"));
  console.log(colors.cyan("─".repeat(50)));
  ...
  if (!options.force) {
    ...
    const answer = await new Promise<string>((resolve) => {
      rl.question(colors.red("🚨 Are you absolutely sure you want to continue? This action cannot be undone. (type 'RESET' to confirm): "), resolve);
    });
